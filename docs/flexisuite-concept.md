...
- **アプリプロセス（ハブ/認証UI/ストア/業務UIなど）**: すべて静的配信主体のフロント。API呼び出しはカーネルのみ。SSRは不要が原則。
- **コンポーネント**: カスタムUXの成果物。UI/サーバー双方あり、単独実行より「アプリプロセスから呼び出される」ことを前提。依存関係とポリシーを伴う。

## カスタムUXライフサイクル（責務分割）
1) 生成: AI/開発者がドラフトコンポーネントを作成（上位サービス側で実施）。
2) テスト: ビルド・テストを実行しレポートを添付（上位サービス）。
3) 承認: 承認API経由でカーネルに署名付きパッケージとして登録（hash+manifestを固定）。
4) 配信: カーネルがレジストリで公開し、署名・ハッシュを検証可能な形で提供。
5) インストール: テナント管理者がコンポーネントをグループにインストール。依存は自動解決しロックファイルを生成。
...
- Registry: register/get/list/revoke component packages, download bundle.
...
- `docs/sandbox-modes.md` で draft/staging/stable の運用モードとリソース制限を定義。（実施済み）
