...
mod tests {
    ...
    fn test_idempotency_canonical_request_target() {
        let cases = vec![
            ...
            ("/v1/search", Some("q=Hello%20World"), "/v1/search?q=Hello%20World"),
        ...
        for (input_path, input_query, expected) in cases {
            ...
            assert_eq!(canonicalize_request_target(input_path, input_query), expected, "Failed for input: {} ? {:?}", input_path, input_query);
    ...
    fn test_idempotency_query_order_conflict_guard() {
         ...
         assert_eq!(target_a, target_b, "Query order difference must be normalized");
         ...
         let is_conflict_1 = check_idempotency_conflict(&target_a, body_a_hash, &target_b, body_b_hash);
         assert!(!is_conflict_1, "Same body should not conflict");

         // Scenario 2: Same Target, Diff Body -> Conflict (409)
         let is_conflict_2 = check_idempotency_conflict(&target_a, body_a_hash, &target_b, body_c_hash);
         assert!(is_conflict_2, "Different body with same canonical target MUST conflict");
         ...
         assert_ne!(target_c, target_d, "Empty segments must be preserved to avoid collision");
    }
