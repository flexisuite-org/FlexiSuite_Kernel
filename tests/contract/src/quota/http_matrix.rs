...
mod tests {
    ...
    fn test_quota_http_matrix() {
        ...
        for scenario in scenarios {
            ...
            let violation = QuotaViolation { layer: scenario.layer, retry_after_s: 10 };
            ...
            assert_eq!(retry_after.unwrap().1, "10", "Retry-After value hash must match");
    ...
    fn test_quota_retry_after_boundaries() {
        ...
        let v_zero = QuotaViolation { layer: QuotaLayer::ApiRateLimit, retry_after_s: 0 };
        let h_zero = v_zero.headers();
        assert_eq!(h_zero[0].1, "0");

        // Case 2: Large value (Generic layer) -> Cap at 1 year
        let v_large = QuotaViolation { layer: QuotaLayer::ApiRateLimit, retry_after_s: 999_999_999 };
        assert_eq!(v_large.headers()[0].1, "31536000");

        // Case 3: SystemHardLimit boundary -> 1-30s clip
        let v_sys_low = QuotaViolation { layer: QuotaLayer::SystemHardLimit, retry_after_s: 0 };
        assert_eq!(v_sys_low.headers()[0].1, "1");
        
        let v_sys_high = QuotaViolation { layer: QuotaLayer::SystemHardLimit, retry_after_s: 100 };
        assert_eq!(v_sys_high.headers()[0].1, "30");
        
        let v_sys_ok = QuotaViolation { layer: QuotaLayer::SystemHardLimit, retry_after_s: 15 };
