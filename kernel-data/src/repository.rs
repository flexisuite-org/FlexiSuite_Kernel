...
impl TenantRepository for TenantScoped<RawConnection> {
    async fn create_entity(&self, mut active_model: entity_record::ActiveModel) -> Result<entity_record::Model, DataError> {
        ...
        let result = active_model.insert(&self.inner.txn).await.map_err(DataError::DbError)?;
        Ok(result)
    }
