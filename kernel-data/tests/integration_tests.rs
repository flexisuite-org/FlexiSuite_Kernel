...
use kernel_data::DataError; // Import DataError
...
async fn test_tenant_isolation_rls() {
    ...
    // Generate Tokens
    let token_a = KeyManager::generate_tenant_token(&db, tenant_a.tenant_id().as_str()).await.expect("Failed to gen token A");
    let token_b = KeyManager::generate_tenant_token(&db, tenant_b.tenant_id().as_str()).await.expect("Failed to gen token B");

    let record_id = Uuid::now_v7().to_string();

    // A. Insert as Tenant A
    let record_id_a = record_id.clone();
    with_tenant_tx(&db, &tenant_a, &token_a, |repo| Box::pin(async move {
    ...
    let record_id_clone = record_id.clone();
    with_tenant_tx(&db, &tenant_b, &token_b, |repo| Box::pin(async move {
        let res = repo.get_entity(&record_id_clone).await?;
...
// Return DataError instead of kernel::Result
